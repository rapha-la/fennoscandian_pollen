# North and South with eco.groups

ecogroupsN_2_CLTV <- read.csv("ecogroupsN_2_CLTV.csv")


#SOUTH
SOUTH <- ecogroupsN_2_CLTV

#SOUTH = subset(NORTH,select= c(...)) ??
...
# Save
write.csv(SOUTH, file = "SOUTH_groups.csv", row.names = FALSE)
SOUTH_groups <- read.csv("SOUTH_groups.csv")


#NORTH
NORTH <- ecogroupsN_2_CLTV
...
# Save
write.csv(NORTH, file = "NORTH_groups.csv", row.names = FALSE)
